...
market_regression <- ggplot(abc, aes(x = pct_soc_spending, y = value)) + 
    ...
    labs(title = "Effect of Social Spending Programs on Labor Market Freedom",
         subtitle = "Labor Market Freedom Measured on 1-5 Scale: Higher Number Means Comparatively Lower Freedom",
         x = "Level of Social Spending as % of GDP",
         y = "Degree of Labor Market Freedom")
...
ad1 <- ad %>%
  ...
  nest()
x <- ad1 %>%
  mutate(mod = map(data, ~ lm(value ~ pension_spending, data = .)),
         reg_results = map(mod, ~ tidy(.)),
         coef = map_dbl(reg_results, ~ filter(., term == "pension_spending") 
                        %>% pull(estimate)),
         se = map_dbl(reg_results, ~ filter(., term == "pension_spending")
                      %>% pull(std.error)),
         upper = coef + 1.96 * se,
         lower = coef - 1.96 * se)
 ggplot(x, aes(x = time, y = coef)) +
  geom_point(color = "blue") +
  geom_errorbar(aes(x = year, ymin = lower, ymax = upper), color = "blue") +
  theme_classic() +
